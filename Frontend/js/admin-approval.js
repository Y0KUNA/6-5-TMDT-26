...
function loadVendors() {
  ...
  // If no vendors in localStorage, use sample data
  if (vendors.length === 0) {
...
function approveVendor(vendorId) {
  ...
  if (vendor) {
    ...
    alert(`Đã phê duyệt đơn đăng ký của ${vendor.businessName}`);
    renderVendors();
  }
}
...
function confirmReject() {
  ...
  if (vendor) {
    ...
    alert(`Đã từ chối đơn đăng ký của ${vendor.businessName}`);
    closeRejectModal();
    renderVendors();
  }
}
