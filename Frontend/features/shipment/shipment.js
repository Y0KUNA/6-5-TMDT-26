...
function checkShipperAuth() {
  ...
  if (role !== 'shipper' || !shipperId) {
    const container = document.getElementById('availableList') || document.getElementById('shipmentsList');
    if (container) {
...
// ============ Tab Management ============
// ✅ Sửa: nhận `btnEl` thay vì dùng `event` global ngầm định
...
function openAcceptModal(shipmentId, orderId, customerName) {
  ...
  document.getElementById('acceptContent').innerHTML = `
    ...
    <div style="background: #f9fafb; padding: 12px; border-radius: 4px; border-left: 4px solid #16a34a;">
  ...
  try {
    ...
    container.innerHTML = shipments.map(s => {
      ...
      return `<div class="shipment-card">
        ...
        <div class="action-buttons">
          <button class="btn-sm btn-next" onclick="updateStatusWithReason(${s.shipment_id}, 'PICKED_UP')">📦 Đã lấy hàng</button>
          <button class="btn-sm btn-next" onclick="updateStatusWithReason(${s.shipment_id}, 'DELIVERING')">🚚 Đang giao</button>
          <button class="btn-sm btn-next" onclick="updateStatusWithReason(${s.shipment_id}, 'DELIVERED')">✓ Giao xong</button>
          <button class="btn-sm btn-fail" onclick="openFailureModal(${s.shipment_id})">✗ Giao thất bại</button>
        </div>
      </div>`;
    }).join('');
  } catch (err) {
